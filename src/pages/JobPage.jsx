...
import DOMPurify from "dompurify";
import Button from "../components/Button";
import { useAuth } from "../context/useAuth"; // Imported your auth hook
...
const JobPage = ({ deleteJob }) => {
  const navigate = useNavigate();
  const job = useLoaderData();
  const { user } = useAuth(); // Destructured user object from context
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            {/* Manage Job panel only displays if the logged in user created the post */}
            {isOwner && (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                <h2 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-4">
                  Manage Job
